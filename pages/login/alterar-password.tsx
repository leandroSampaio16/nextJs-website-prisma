            ...
            <main className="bg-background w-full mx-auto flex justify-center">
                <div className="w-[25rem] h-[35rem] bg-branco flex flex-row rounded-2xl lg:w-[50rem] lg:h-[35rem] my-5" style={{ boxShadow: "0 0 4px 0 rgba(0, 0, 0, 0.10) inset" }}>
                    ...
                    <div className="w-full lg:w-1/2 my-5 sm:m-5">
                        <br />
                        <br />
                        <h1 className="text-preto font-bold text-3xl mb-3 text-center lg:text-left">Altere a senha!</h1>
                        <h2 className="text-preto text-lg text-center lg:text-left">Coloque a sua nova senha aqui</h2>
                        <br />
                        <br />
                        <form className="flex flex-col justify-center items-center w-full" method="POST" action={"/api/esqueceu/alterar"}>
                            ...
                            <input className="w-3/4 mx-auto p-1 px-3 rounded-2xl outline-none border-2 border-preto border-opacity-10" type={verSenha ? "text" : "password"} placeholder="Repetir Senha" name="rsenha" required />
                            <br />
                            <input type="submit" value={"Alterar"} className="w-3/4 p-3 border border-cor4 bg-cor4 font-semibold text-branco text-xl rounded-xl hover:bg-transparent hover:text-preto transition-colors duration-200" />
                        </form>
                        <Link className="mx-auto w-3/4 block text-left text-preto text-opacity-50 text-sm hover:text-opacity-100 transition-all duration-200" href={"/login"}>Não necessita de recuperar conta?</Link>
                        <br />
                    </div>
                </div>
            </main>
